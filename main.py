...
from config.model_config import (
    ...
    ACTIVITY_DISPLAY_MIN_CONFIDENCE,
    ACTIVITY_RATE_HZ,
...
def build_specs(
    ...
    if not specs:
        sys.exit(
            f"配置里没有可用的 objects: {args.config}"
        )
...
def cfg_value(
    ...
    return (
        default
        if args_value is None and config_value is None
        else (
            config_value
...
def draw_activity_panel(
    ...
    cv2.putText(
        canvas,
        (
            f"State: {engine.last_state} | "
            f"Engine: {engine.total_event_count} events"
        ),
        (x, y),
        font,
        0.52,
...
def main():
    ...
    parser.add_argument(
        ...
        help=(
            "YOLO-World 模型规模；"
            "默认读取 YAML vision.object_detector.model_size"
        ),
    )
    ...
    parser.add_argument(
        ...
        help=(
            "YOLO 输入尺寸；"
            "默认读取 YAML vision.object_detector.imgsz"
        ),
    )
    ...
    parser.add_argument(
        ...
        help=(
            "推理设备；"
            "默认读取 YAML vision.object_detector.device"
        ),
    )
    ...
    parser.add_argument(
        ...
        help=(
            "检测框去重 IoU；"
            "默认读取 YAML vision.object_detector.dup_iou"
        ),
    )
    ...
    parser.add_argument(
        ...
        help=(
            "每 N 帧运行一次 YOLO；"
            "默认读取 YAML vision.tracker.detect_every"
        ),
    )
    ...
    parser.add_argument(
        ...
        help=(
            "轨迹匹配 IoU；"
            "默认读取 YAML vision.tracker.match_iou"
        ),
    )
    ...
    parser.add_argument(
        ...
        help=(
            "最大滑行帧数；"
            "默认读取 YAML vision.tracker.max_coast_frames"
        ),
    )
    ...
    parser.add_argument(
        ...
        help=(
            "轨迹确认所需命中次数；"
            "默认读取 YAML vision.tracker.min_hits"
        ),
    )
    ...
    parser.add_argument(
        ...
        help=(
            "手部模型路径；"
            "默认读取 YAML vision.hand_detector.model_path"
        ),
    )
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Activity Engine 输入频率；"
            "默认读取 YAML runtime.activity_rate_hz"
        ),
    )
    ...
    # 没有 CLI 参数时，从 config/model_config.py 获取默认值。
    # ========================================================
    ...
    args.activity_min_confidence = (
        ACTIVITY_DISPLAY_MIN_CONFIDENCE
        if args.activity_min_confidence is None
        else args.activity_min_confidence
    )
    ...
    args.vlm_cooldown = (
        VLM_COOLDOWN
        if args.vlm_cooldown is None
        else args.vlm_cooldown
    )

    args.vlm_events = (
        ",".join(sorted(VLM_TRIGGER_EVENTS))
        if args.vlm_events is None
        else args.vlm_events
    )

    args.activity_min_confidence = max(
        0.0,
        min(
            1.0,
            float(args.activity_min_confidence),
        ),
    )
    ...
    args.hand_model = str(
        resolve_project_path(
            cfg_value(
                args.hand_model,
                hand_cfg.get("model_path"),
                HAND_MODEL_PATH,
            )
        )
    )

    args.activity_rate = float(
        cfg_value(
            args.activity_rate,
            runtime_cfg.get("activity_rate_hz"),
            ACTIVITY_RATE_HZ,
        )
    )
    ...
    if (
        ...
        camera_fps = CAMERA_FALLBACK_FPS
    ...
    if args.activity_jsonl:
        ...
        perception_path.parent.mkdir(
            parents=True,
            exist_ok=True,
        )

        perception_file = (
            perception_path.open(
                "w",
                encoding="utf-8",
            )
        )
    ...
    if not args.no_vlm:
        ...
        try:
            ...
            print(
                ...
                f"cooldown="
                f"{args.vlm_cooldown:.1f}s, "
                f"events="
                f"{sorted(trigger_events)}"
            )
    ...
    print(
        ...
        f"match_iou={args.match_iou:.2f}, "
        f"max_coast={args.max_coast}, "
        f"min_hits={args.min_hits}"
    )

    print(
        f"Activity Engine: "
        f"{activity_rate:.1f}Hz, "
